...
const prisma = require('../prisma/prismaClient'); // Asegúrate de que el path de prismaClient sea correcto
...
router.post('/servicios', async (req, res) => {
  const { titulo, descripcion, imagen, categoria_id } = req.body; // Asegúrate de que el campo es `categoria_id`
  try {
  ...
  try {
    const servicioActualizado = await prisma.servicios.update({
      ...
      data: {
        ...
        imagen,
        categoria: {
          connect: { id: categoria_id }, // Asegúrate de usar `categoria_id` aquí también
        },
      },
    });
