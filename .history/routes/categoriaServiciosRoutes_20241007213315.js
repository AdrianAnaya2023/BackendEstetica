...
router.get('/categorias-servicios', async (req, res) => {
  try {
    ...
    // Convertir el campo `BigInt` a string
    const modifiedCategorias = categorias.map(categoria => ({
      ...categoria,
      id: categoria.id.toString(), // Aquí convertimos el BigInt a String
    }));

    res.json(modifiedCategorias); // Devolvemos el resultado modificado
  } catch (error) {
