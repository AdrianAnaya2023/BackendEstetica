...
router.post('/', validateServiceData, async (req, res) => {
  try {
    ...
    // Convertir BigInt a string para todos los campos relevantes
    const responseData = {
      ...nuevoServicio,
      id: nuevoServicio.id.toString(),
      categoria_id: nuevoServicio.categoria_id.toString(), // Asegúrate de que este campo esté presente
    };
    // Si hay alguna relación que también contenga BigInt, conviértelo aquí
    if (nuevoServicio.categoria) {
      responseData.categoria = {
        ...nuevoServicio.categoria,
        id: nuevoServicio.categoria.id.toString(),
      };
    }
